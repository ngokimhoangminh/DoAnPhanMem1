    ...
    <div class="col-lg-12">
            <section class="panel">
                ...
                <div class="panel-body">
                   @foreach($blood_actual as $key => $value_updates)
                    <div class="position-center">
                        <form role="form">
                            <div class="form-group">
                                <label for="exampleInputEmail1">Đợt Hiến Máu</label>
                                <input type="text" name="blood_donation_name" disabled value="{{$value_updates['blood_name']}}" class="form-control">
                                <input type="hidden" name="" class="signup_blood_id" value="{{$signup_blood_id}}">
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">Họ Và Tên</label>
                                <input type="text" name="users_fullname" disabled value="{{$value_updates['users_fullname']}}" class="form-control">
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1" style="font-size: 14px;font-family: initial;">Nhóm Máu</label>
                                <input type="text" value="{{$value_updates['blood_actual_group']}}" class="form-control blood_actual_group" placeholder="Enter Employee">
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">Đơn Vị Máu</label>
                                <input type="text" value="{{$value_updates['blood_actual_unit']}}" name="blood_actual_unit" class="form-control blood_actual_unit">
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">Tình Trạng Sức Khỏe</label>
                                <input type="text" name="blood_actual_health" value="{{$value_updates['blood_actual_health']}}" class="form-control blood_actual_health">
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">Tình Huống Phát Sinh</label>
                                <input type="text" name="blood_actual_situations" value="{{$value_updates['blood_actual_situations']}}" class="form-control blood_actual_situations">
                            </div>
                            <button type="button" class="btn btn-info update_employee" onclick="edit({{$value_updates['blood_actual_id']}})" name="update_employee">Cập nhật</button>
                        </form>
                    </div>
                @endforeach
                </div>
