...
class BloodDonationController extends Controller
{
    ...
    public function list_blooddonation()
    {
        ...
        ->orderBy('tbl_blood_donation.blood_finish_date','desc')
        ->get();
    ...
    public function report_list_blood($bloodid)
    {
        $blood_donation=BloodDonation::where('blood_donation_id',$bloodid)->first();
        

        $signup_blood=SignupBlood::where('blood_donation_id',$bloodid)->where('signup_blood_status',1)->get();
        $signup_blood_id=[];
        $data=[];
        foreach ($signup_blood as $key => $value) {
            $users=Users::where('users_id',$value['users_id'])->first();
            ...
            [
                ...
                'users_address'=>$users->users_address,
                'users_phone'=>$users->users_phone,
                'blood_group'=>$blood_actual->blood_actual_group,
                'blood_actual_unit'=>$blood_actual->blood_actual_unit,
                'blood_actual_health'=>$blood_actual->blood_actual_health
            ];
        }
        View::share('blood_name',$blood_donation->blood_donation_name);
        View::share('dataReport',$data);
